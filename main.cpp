...
void side(int t)    
{
    ...
    {             
        u[i][-1+bd]=u[i][1+bd]; v[i][-1]=-v[i][1+bd]; pres[i][-1+bd]=pres[i][1+bd]; rho[i][-1+bd]=rho[i][1+bd]; cc[i][-1+bd]=cc[i][1+bd];
        u[i][-2+bd]=u[i][2+bd]; v[i][-2]=-v[i][2+bd]; pres[i][-2+bd]=pres[i][2+bd]; rho[i][-2+bd]=rho[i][2+bd]; cc[i][-2+bd]=cc[i][2+bd];
        u[i][-3+bd]=u[i][3+bd]; v[i][-3]=-v[i][3+bd]; pres[i][-3+bd]=pres[i][3+bd]; rho[i][-3+bd]=rho[i][3+bd]; cc[i][-3+bd]=cc[i][3+bd];
        u[i][-4+bd]=u[i][4+bd]; v[i][-4]=-v[i][4+bd]; pres[i][-4+bd]=pres[i][4+bd]; rho[i][-4+bd]=rho[i][4+bd]; cc[i][-4+bd]=cc[i][4+bd];
}
    for (i=61+bd;i<=300+bd;i++)
    {            
...
void Steger_Warming_X(double rho,double u,double v,double c,int i,int j)   
{	
    lmd[1]=u;
    lmd[2]=lmd[1];
    lmd[3]=lmd[1]-c;
...
void Steger_Warming_Y(double rho,double u,double v,double c,int i,int j)   
{	
    miu[1]=v;
    miu[2]=miu[1];
    miu[3]=miu[1]-c;
...
double WENO_X(int i,int j,int k)      
{
    qx0p[k]=(1.0/3.0)*fplus[i-2][j][k]-(7.0/6.0)*fplus[i-1][j][k]+(11.0/6.0)*fplus[i][j][k];        
...
double WENO_Y(int i,int j,int k)     
{
    qy0p[k]=(1.0/3.0)*gplus[i][j-2][k]-(7.0/6.0)*gplus[i][j-1][k]+(11.0/6.0)*gplus[i][j][k];        
